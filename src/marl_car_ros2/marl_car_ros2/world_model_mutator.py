...
class WorldModelMutator(Node):
    """
    Non-stationary perturbation injector:
    1) Ghost crossing: spawn or reposition a dynamic obstacle crossing ego path.
    2) Extreme road: temporarily lower ground friction.
    """

    def __init__(self) -> None:
        super().__init__("world_model_mutator")
        ...
        self.get_logger().info(f"world mutator ready, gazebo_msgs available={GAZEBO_AVAILABLE}")
    ...
    def _tick(self) -> None:
        ...
        p = random.random()
        if p < self.ghost_prob:
            self._trigger_ghost_probe(now)
        elif p < self.ghost_prob + self.friction_prob:
    ...
    def _trigger_ghost_probe(self, now_ts: float) -> None:
        ...
        crossing_x = ex + random.uniform(1.8, 3.2)
        crossing_y = ey + random.choice([-1.0, 1.0]) * random.uniform(0.6, 1.2)
        direction = -1.0 if crossing_y > ey else 1.0

        severity = random.uniform(0.55, 0.95)
    ...
    def _trigger_friction_drop(self, now_ts: float) -> None:
        ...
        self.last_event_ts = now_ts
        self.friction_restore_at = now_ts + self.friction_duration_s
