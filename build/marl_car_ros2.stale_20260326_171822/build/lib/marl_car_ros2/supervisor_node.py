...
class SupervisorNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("hard_stop_range", 0.12)
        ...
        self.declare_parameter("mode_cautious_range", 0.25)
        self.declare_parameter("mode_medium_range", 0.45)
        ...
        self.benchmark_mode = bool(self.get_parameter("benchmark_mode").value)
    ...
    def _transition_mode(
        ...
        if self.benchmark_mode:
            ...
            if self.mode_state != ManagerMode.NOMINAL:
                self.mode_enter_wall = now
            return ManagerMode.NOMINAL, "benchmark_pass_through"
        ...
        if self.mode_state == ManagerMode.BLOCKED:
            if advisory_mode not in (ManagerMode.BLOCKED, ManagerMode.RECOVERY_REQUESTED):
                if risk_level >= 2 or advisory_mode == ManagerMode.CAUTIOUS or trend_fail:
                    self.mode_enter_wall = now
                    return ManagerMode.CAUTIOUS, "blocked_cleared"
                self.mode_enter_wall = now
                return ManagerMode.NOMINAL, "blocked_cleared"
